...
document.addEventListener('DOMContentLoaded', () => {
  ...
  widget.addEventListener('click', (e) => {
    ...
    const key = step === 1 ? 'occasion' : step === 2 ? 'mood' : 'type';
  ...
  function scoreProduct(product) {
    let score = 0;
    const occasionRules = QUIZ_RULES[answers.occasion];
    const typeRules = QUIZ_RULES[answers.type];
    const moodRules = QUIZ_RULES[answers.mood];

    if (occasionRules?.types?.some((t) => product.type?.includes(t))) score += 3;
    if (typeRules?.types?.some((t) => product.type?.includes(t))) score += 4;
    if (occasionRules?.tags?.some((t) => product.tags?.some((tag) => tag.includes(t)))) score += 2;
    if (moodRules?.tags?.some((t) => product.tags?.some((tag) => tag.includes(t)))) score += 3;
